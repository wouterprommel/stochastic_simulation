'''
This module contains functions for comparing and 
plotting the performance of different sampling methods
'''
import matplotlib.pyplot as plt
from mpl_toolkits.axes_grid1.inset_locator import zoomed_inset_axes, mark_inset
import mandelbrot

sample_sizes = [10**2, 20**2, 30**2, 40**2, 50**2, 60**2, 70**2, 80**2, 90**2, 100**2,
                200**2, 300**2, 400**2, 500**2, 600**2, 700**2, 800**2, 900**2, 1000**2]
...
def plot_comparison(sizes, max_iter):
    '''
    Plot the estimated area of the Mandelbrot set for different sample sizes the four sampling methods.
    ...
    ax.errorbar(sizes, uniform_areas, yerr=uniform_errors, label='Uniform sampling', color=colors[0], marker='s', linestyle='-', CAPsize = CAP, markersize=MARKER_SIZE, linewidth = LW, alpha = ALPHA)
    ax.errorbar(sizes, hypercube_areas, yerr=hypercube_errors, label='Hypercube sampling', color=colors[1], marker='o', linestyle='-', CAPsize = CAP, markersize=MARKER_SIZE, linewidth = LW, alpha = ALPHA)
    ax.errorbar(sizes, ortho_areas, yerr=ortho_errors, label='Orthogonal sampling', color=colors[2], marker='^', linestyle='-', CAPsize = CAP, markersize=MARKER_SIZE, linewidth = LW, alpha = ALPHA)
    ax.errorbar(sizes, masking_areas, yerr=masking_errors, label='Custom sampling', color=colors[4], marker='x', linestyle='-', CAPsize = CAP, markersize=MARKER_SIZE, linewidth = LW, alpha = ALPHA)
    ...
    axins.errorbar(sizes, uniform_areas, yerr=uniform_errors, color=colors[0], marker='s', linestyle='-', CAPsize = CAP, markersize=MARKER_SIZE, linewidth = LW, alpha = ALPHA)
    axins.errorbar(sizes, hypercube_areas, yerr=hypercube_errors, color=colors[1], marker='o', linestyle='-', CAPsize = CAP, markersize=MARKER_SIZE, linewidth = LW, alpha = ALPHA)
    axins.errorbar(sizes, ortho_areas, yerr=ortho_errors, color=colors[2], marker='^', linestyle='-', CAPsize = CAP, markersize=MARKER_SIZE, linewidth = LW, alpha = ALPHA)
    axins.errorbar(sizes, masking_areas, yerr=masking_errors, color=colors[4], marker='x', linestyle='-', CAPsize = CAP, markersize=MARKER_SIZE, linewidth = LW, alpha = ALPHA)
    ...
    ax.set_xscale('log')
    ax.grid(True)
    plt.savefig(f'Figures/Estimated_Area_vs_Number_of_Samples_{max_iter}_iterations.pdf',
